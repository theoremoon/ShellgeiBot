...
func InsertResult(db *sql.DB, tweet_id int64, result string, b64img string, err error) error {
	err_str := ""
	if err != nil {
		err_str = err.Error()
	}
	_, err2 := db.Exec("update shellgeis set result=?, b64img=?, error=? where tweet_id=?", result, b64img, err_str, tweet_id)
	return err2
}
